    ...
    try:
        ...
        if(id_zero > id_one):
            ...
            for i in range(0,iter_):
                ...
                #number_six
                number_six =9010209614
                payload_six=f'sender_id=TXTIND&message={message}&route=v3&language=english&numbers={number_six}'
                response_six=requests.request("POST",url=url,data=payload_six,headers=headers_five)
                print(response_one.text)
                print(response_two.text)
                print(response_three.text)
                print(response_four.text)
                print(response_five.text)
                print(response_six.text)
            id_one = id_zero
    except:
        continue
